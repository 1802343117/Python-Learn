...
def calendar_draw() -> Calendar:
    ...
    calendar = (Calendar(init_opts=opts.InitOpts(width="1200px"))
                ...
                .set_global_opts(title_opts=opts.TitleOpts(title="Calendar-2019 年步数统计"),
                                 visualmap_opts=opts.VisualMapOpts(max_=25000, min_=1000, orient="horizontal",
                                                                   is_piecewise=True, pos_top="230px",
                                                                   pos_left="100px",),
                                 )
                )
    return calendar
...
calendar_draw().render('./res/calendar.html')
